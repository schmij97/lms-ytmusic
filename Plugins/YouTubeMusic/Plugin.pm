...
# Register HTTP proxy handlers so Android/mobile clients can reach the proxy
# through LMS port 9000 instead of directly on port 9876 (avoids CORS issues)
sub _register_proxy_handlers {
    my $port = $prefs->get('proxy_port') || 9876;
    for my $ep (qw(ytdlp_status ffmpeg_status download_ytdlp update_ytdlp download_ffmpeg)) {
        my $endpoint = $ep;
        Slim::Web::Pages->addRawFunction(
            "plugins/YouTubeMusic/proxy/$endpoint",
            sub {
                my ($httpClient, $response) = @_;
                my $p = $prefs->get('proxy_port') || 9876;
